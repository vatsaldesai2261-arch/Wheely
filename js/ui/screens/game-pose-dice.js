...
export default {
  ...
  onLeave() { if (rolling) { clearInterval(rolling); rolling = null; } speech.stop(); endPlay(); },
};
...
function land(stage, pose) {
  ...
  const did = el('button.btn.btn-primary', { type: 'button', onClick: (e) => { rewardPose(pose); e.currentTarget.disabled = true; e.currentTarget.textContent = 'Great job! ⭐'; } }, 'I did it! ⭐');
  stage.append(el('div.game-panel', {}, [
    ...
    el('div.game-actions', {}, [
      did,
      el('button.btn.btn-secondary.btn-xl', { type: 'button', onClick: () => roll(stage) }, 'Roll again 🎲'),
      homeBtn(),
    ]),
  ]));
}
